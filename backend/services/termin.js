...
serviceRouter.get('/termin/kunde/:id', function(request, response) {
    console.log('Service Termin: Client requested all records for kunde,  id=' + request.params.id);

    const terminDao = new TerminDao(request.app.locals.dbConnection);
    try {
        var arr = terminDao.loadAllByKunde(request.params.id);
        console.log('Service Termin: Records loaded, count=' + arr.length);
        response.status(200).json(arr);
    } catch (ex) {
        console.error('Service Termin: Error loading all records by kunde. Exception occured: ' + ex.message);
        response.status(400).json({ 'fehler': true, 'nachricht': ex.message });
    }
});
...
serviceRouter.post('/termin', function(request, response) {
    ...
    if (helper.isUndefined(request.body.kunde)) {
        request.body.kunde = null;
    } else if (helper.isUndefined(request.body.kunde.id)) {
        errorMsgs.push('kunde gesetzt, aber id fehlt');
    } else {
        request.body.kunde = request.body.kunde.id;
    }
    ...
    const terminDao = new TerminDao(request.app.locals.dbConnection);
    try {
        var obj = terminDao.create(request.body.bezeichnung, request.body.beschreibung, helper.parseDateTimeString(request.body.zeitpunkt), request.body.dauer, request.body.dienstleister, request.body.kunde);
...
serviceRouter.put('/termin', function(request, response) {
    ...
    const terminDao = new TerminDao(request.app.locals.dbConnection);
    try {
        var obj = terminDao.update(request.body.id, request.body.bezeichnung, request.body.beschreibung, helper.parseDateTimeString(request.body.zeitpunkt), request.body.dauer, request.body.dienstleister, request.body.kunde);
